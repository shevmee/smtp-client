...
{
    ISXSC::MailMessage message;
    message.from = { "tom.doe@gmail.com", "Tom Doe" };
    std::string expected = "From: Tom Doe <tom.doe@gmail.com>\r\n";

    EXPECT_EQ(ISXSC::MailMessageFormatter::MailFrom(message.from), expected);
}
...
TEST(MailMessageFormatter, FormatMessageWithMultipleRecipients)
{
    ...
    EXPECT_EQ(ISXSC::MailMessageFormatter::MailTo(message.to), expected);
}
...
TEST(MailMessageFormatter, FormatMessageWithCc)
{
    ...
    EXPECT_EQ(ISXSC::MailMessageFormatter::MailCc(message.cc), expected);
}
...
TEST(MailMessageFormatter, FormatMessageHeaders)
{
    ...
    EXPECT_EQ(ISXSC::MailMessageFormatter::MailAttachmentHeaders(message.attachments[0]), expect_first);
    EXPECT_EQ(ISXSC::MailMessageFormatter::MailAttachmentHeaders(message.attachments[1], "text/plain"), expect_second);
}
