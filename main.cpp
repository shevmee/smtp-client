...
using std::cout;
void finish(asio::io_context &io_context, std::thread &worker,
            std::unique_ptr<ISXSC::SmtpClient> &smtp_client);
...
int main() {
  ...
  // Make io_context run in a separate thread and event loop poll for new async
  // operations
  ...
  std::unique_ptr<ISXSC::SmtpClient> smtp_client =
      std::make_unique<ISXSC::SmtpClient>(io_context, ssl_context);
